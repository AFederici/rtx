...
View::View(int width, int height, double sideLen, double dist){
    h_res = width;
    v_res = height;
    pixel_size = sideLen;
    d = dist;
...
vector<Point> View::getSingleRay(int col, int row){ 
    vector<Point> ret;
    ret.push_back(getLookAt(col, row, 0.5, 0.5));
    return ret;
}

vector<Point> View::getMultiJittered(int col, int row, int sampling_rate){
    vector<Point> ret;
    ...
    for (int coarse_row = 0; coarse_row < gran; coarse_row++){
        for (int coarse_col = 0; coarse_col < gran; coarse_col++){
            ...
            ret.push_back(getLookAt(col, row, x_off, y_off));
...
//x is columns, horizontal
//y is rows, vertical
Point View::getLookAt(int col, int row, double x_off = 0.5, double y_off = 0.5) { return Point(pixel_size * (col - (h_res/2.0) + x_off), pixel_size * (row - (v_res/2.0) + y_off), -d); }